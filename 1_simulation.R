...
cat("run analysis on simulation data\n")
...
m_trap_age <- cstan( file= "models/1_trap_age.stan" , data=dat_traps , chains=3, cores = 3, iter = 1000 )
...
m_shells_all <- cstan( file= "models/9_shells_estimate_all_negative_effects.stan" , data=dat_shells , 
...
for (i in 1:4) {
  phi_min <-  apply(post_t$iota,1,mean )  +
    ...
    post_t$eta_h* ifelse( i == 1, min(log(dat_traps$height), na.rm = TRUE), mean(log(dat_traps$height), na.rm = TRUE) ) +
    post_t$theta_g* ifelse( i == 2, min(log(dat_traps$grip), na.rm = TRUE), mean(log(dat_traps$grip), na.rm = TRUE) )  +
  ...
  phi_max <-  apply(post_t$iota,1,mean )  +
    post_t$gamma * log(1-exp(- post_t$beta * 1.6  )) +
    post_t$eta_h * ifelse( i == 1, max(log(dat_traps$height), na.rm = TRUE), mean(log(dat_traps$height), na.rm = TRUE) ) +
    post_t$theta_g* ifelse( i == 2, max(log(dat_traps$grip), na.rm = TRUE), mean(log(dat_traps$grip), na.rm = TRUE) ) +
...
pars <- data.frame( g_g = c(2, 1, 3,-1, 4,-3),
                    g_h = c(2, 4,-4,-3, 0, 1),
                    g_k = c(2,-2, 5, 0,-3, 4))
...
legend(min(pars)-0.8, max(pars)+1.5, legend = c("g", "h","k"), col = othercol, pch=15:17)
...
for ( j in 1:nrow(pars)){
  ...
  points(pars[j,"g_g"], mean(post_s$theta_g), col = col.alpha(shellcol, 0.6), pch = 15)
  points(pars[j,"g_h"], mean(post_s$eta_h), col = col.alpha(shellcol, 0.6), pch = 16)
  points(pars[j,"g_k"], mean(post_s$zeta_k), col = col.alpha(shellcol, 0.6), pch = 17)
  points(pars[j,"g_g"], mean(post_t$theta_g), col = col.alpha(trapcol, 0.6), pch = 15)
  points(pars[j,"g_h"], mean(post_t$eta_h), col = col.alpha(trapcol, 0.6), pch = 16)
  points(pars[j,"g_k"], mean(post_t$zeta_k), col = col.alpha(trapcol, 0.6), pch = 17)
  lines(rep(pars[j,"g_g"], 2), PI(post_s$theta_g), col = col.alpha(shellcol, 0.8))
  lines(rep(pars[j,"g_h"], 2), PI(post_s$eta_h), col = col.alpha(shellcol, 0.8))
  lines(rep(pars[j,"g_k"], 2), PI(post_s$zeta_k), col = col.alpha(shellcol, 0.8))
  lines(rep(pars[j,"g_g"], 2), PI(post_t$theta_g), col = col.alpha(trapcol, 0.8))
  lines(rep(pars[j,"g_h"], 2), PI(post_t$eta_h), col = col.alpha(trapcol, 0.8))
  lines(rep(pars[j,"g_k"], 2), PI(post_t$zeta_k), col = col.alpha(trapcol, 0.8))
