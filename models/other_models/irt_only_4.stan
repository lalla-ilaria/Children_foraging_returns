...
transformed parameters{
  ...
  vector<lower=0>[N] knowledge_stnd;
  vector[O] delta_j;
  ...
  for(i in 1:N) {
    ...
    if (knowledge_impute[i] == 1) knowledge_merged[i] = omega + ro_age[sex[i]] * sum (delta_j[ 1 : age_int[i]] );
  }//i
  for ( i in 1:N ) knowledge_stnd[i] = (knowledge_merged[i] - min(knowledge_merged)) / mean ((knowledge_merged - min(knowledge_merged)));
}
model {
