...
parameters{
  ...
  real<lower=0> sigma_knowledge;  //sd for iota_irt
  vector<lower=0>[2] omicron;   //max effect of age, sex specific
...
transformed parameters{
  ...
  for(i in 1:N) {
    phi[i] = iota[i] * sigma_i + 
             ...
             zeta_k*knowledge[i];//we get knowledge out of log cause it's like we exponentiate knowledge
    }
  for(i in 1:M) {
    psi[i] =  xi*log(duration[i]) + tide[i] * tau; //height of tide
    }
}//transformed parameters
model{
